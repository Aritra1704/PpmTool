...
class UpdateProjectTask extends Component {
    constructor(props) {
        ...
        if(!path.includes("/updateProjectTask"))  {// workaround to get correct params
            window.location.reload(false);
        } else {
            const pathSplit = getId(props);
            backlog_id = pathSplit[pathSplit.length - 2];
            project_seq = pathSplit[pathSplit.length - 1];
        } 

        this.state = {
            "id": "",
            "projectSequence": project_seq,
            "summary": "",
            "acceptanceCriteria": "",
            "status": "",
            "priority": 0,
    ...
    componentDidMount() {
        ...
        const backlog_id = pathSplit[pathSplit.length - 2];
        const projectTaskId = pathSplit[pathSplit.length - 1];
        this.props.getProjectTask(backlog_id, projectTaskId, this.props.history);
    }
